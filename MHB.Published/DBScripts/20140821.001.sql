USE [Test01Db]
GO
/****** Object:  StoredProcedure [dbo].[spGetFrequentlyPurchasedProductsByDate]    Script Date: 08/22/2014 08:51:12 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	(
		...
		(
			...
			WHERE mt.UserID = 1 AND mt.Month = 8 AND mt.Year = 2014 AND mt.IsDeleted = 0 AND dt.IsDeleted = 0 AND dt.ProductID <> 1 AND dt.MeasureTypeID = 3
		...
		(
			...
			INNER JOIN ' + CAST(@mainTableName AS NVARCHAR) + ' mt ON mt.ID = dt.ExpenditureID
			INNER JOIN tbVendors v ON v.VendorID = dt.SupplierID
			WHERE mt.UserID = 1 AND mt.Month = 8 AND mt.Year = 2014 AND mt.IsDeleted = 0 AND dt.IsDeleted = 0 AND dt.ProductID <> 1 AND (dt.MeasureTypeID = 1 OR dt.MeasureTypeID = 2)
			GROUP BY dt.ProductID, dt.DetailName, v.Name
		) t2
	) t3
