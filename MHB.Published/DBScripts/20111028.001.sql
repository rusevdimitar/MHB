USE [Test01Db]
GO
/****** Object:  StoredProcedure [dbo].[spDuplicateMonthRecords]    Script Date: 10/28/2011 17:25:32 ******/
SET ANSI_NULLS ON
GO
...
-- Modified:	28.10.2011
-- =============================================
CREATE PROCEDURE [dbo].[spDuplicateMonthRecords]
...
IF @FlaggedOnly = 1
	BEGIN
		...
		FROM ' + CAST(@MainTableName AS NVARCHAR) + ' WHERE UserID = ' + CAST(@UserID AS NVARCHAR) + ' AND Month = ' + CAST(@CurrentMonth AS NVARCHAR) + ' AND Year = ' + CAST(@CurrentYear AS NVARCHAR) + ' AND Flagged = 1)'
	END
ELSE
BEGIN
	SET @qry = 'INSERT INTO tbMainTableTemp(UserID, Month, Year, FieldName, FieldDescription, FieldValue, FieldExpectedValue, DueDate, IsPaid, HasDetails, AttachmentFileType, HasAttachment, CostCategory, Notified, NotificationDate, Flagged, IsDeleted, FieldOldValue) 
	(SELECT UserID, Month, Year, FieldName, FieldDescription, FieldValue, FieldExpectedValue, DueDate, IsPaid, HasDetails, AttachmentFileType, HasAttachment, CostCategory, Notified, NotificationDate, Flagged, IsDeleted, FieldOldValue
	FROM ' + CAST(@MainTableName AS NVARCHAR) + ' WHERE UserID = ' + CAST(@UserID AS NVARCHAR) + ' AND Month = ' + CAST(@CurrentMonth AS NVARCHAR) + ' AND Year = ' + CAST(@CurrentYear AS NVARCHAR) + ')'
END
