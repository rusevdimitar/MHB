USE [Test01Db]
GO
/****** Object:  StoredProcedure [dbo].[spDeleteDemoEntries]    Script Date: 06/06/2014 10:41:38 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
   ...
   SET @interval = (SELECT DATEDIFF(MINUTE, 0, GETDATE()) - DATEDIFF(MINUTE, 0, MAX(logDate)) FROM dbo.tbActionLog WHERE logAction = 22)
	...
	IF @interval > 15 OR (SELECT COUNT(ID) AS Savings FROM tbMonthlySavings WHERE UserID = 25) > 1
	BEGIN
...
--DELETE FROM tbTransactionLog WHERE UserID = 25
