CREATE TYPE dbo.CategorySelectionType AS TABLE
(
...
USE [Test01Db]
GO
/****** Object:  StoredProcedure [dbo].[spGetAllUsersCategorySums]    Script Date: 08/14/2014 13:18:17 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SELECT ID FROM tbCategories WHERE (UserCategoryID = @userID OR UserCategoryID = 0) AND ID IN (SELECT ID FROM @selectedCategories)
	...
	FETCH NEXT FROM CursorCategoriesIDs INTO @categoryID
		...
		FETCH NEXT FROM CursorCategoriesSum INTO @mainTableName
			...
			BEGIN
				...
				IF @userID = 0
				BEGIN
					SET @qry = N'SET @currentCategorySum = (SELECT ISNULL(SUM(FieldValue), 0) FROM ' + CAST(@mainTableName AS NVARCHAR) + ' WHERE CostCategory = ' + CAST(@categoryID AS NVARCHAR) + ' AND [Year] = ' + CAST(@year AS NVARCHAR) + ' AND IsDeleted = 0)'				
				END
				ELSE 
				BEGIN
					SET @qry = N'SET @currentCategorySum = (SELECT ISNULL(SUM(FieldValue), 0) FROM ' + CAST(@mainTableName AS NVARCHAR) + ' WHERE CostCategory = ' + CAST(@categoryID AS NVARCHAR) + ' AND [Year] = ' + CAST(@year AS NVARCHAR) + ' AND UserID = ' + CAST(@userID AS NVARCHAR) + ' AND IsDeleted = 0)'
				END
